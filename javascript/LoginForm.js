(function(EventEmitter, tmpl, Cognito){
  ...
  function handleChangeLink(event) {
    event.preventDefault();
    EventEmitter.emit('LoginForm:unmount')
    EventEmitter.emit('ChangeForm:mount')
  }

  function handleForgotLink(event) {
    event.preventDefault();
    EventEmitter.emit('LoginForm:unmount')
    EventEmitter.emit('ForgotForm:mount')
  }
  ...
  function handleSubmit(event) {
    ...
    .then(function(result) {
      ...
      setTimeout(redirectToWelcomePage, 500)
      console.log(result)
    })
  ...
  EventEmitter.on('LoginForm:mount', function(message) {
    ...
    .then(function() {
      ...
      $link = $container.getElementsByClassName('Control__link signup')[0];
      ...
      $changelink = $container.getElementsByClassName('Control__link change')[0];
      $changelink.addEventListener('click', handleChangeLink);

      $forgotlink = $container.getElementsByClassName('Control__link forgot')[0];
      $forgotlink.addEventListener('click', handleForgotLink);
